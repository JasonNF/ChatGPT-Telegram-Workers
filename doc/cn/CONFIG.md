...
Telegram 紧凑尾注可使用 `ENABLE_SHOWINFO=true`、`SHOW_PARTS=model,model_time,first_chunk_time,token` 与 `LOG_POSITION_ON_TOP=false`。尾注第一行显示模型、总耗时和 TTFC，第二行显示输入、缓存、输出及思考 Token。
