...
| TELEGRAM_THINKING_DRAFT_ENABLED | 可编辑思考动画 | `true`                                          | 使用可编辑消息展示动画，正式回复会复用同一消息气泡                                                                                                                                                                                                                          |
| TELEGRAM_THINKING_INTERVAL   | 思考动画间隔          | `900` (number)                                  | 点动画的帧间隔，单位毫秒                                                                                                                                                                                                                                                                        |
| TELEGRAM_THINKING_LABEL      | 思考标识文字          | `''`                                            | 动态点前显示的可选名称；留空时只显示图标和动态点                                                                                                                                                                                                                                                          |
| TELEGRAM_THINKING_CUSTOM_EMOJI_ID | MOSS 动态图标       | `''`                                            | Telegram `custom_emoji_id`；同时用于思考动画和最终回复尾标，留空时思考动画显示红色圆点                                                                                                                                                                                                                                                     |
...
Telegram 紧凑尾注可使用 `ENABLE_SHOWINFO=true`、`SHOW_PARTS=model,model_time,first_chunk_time,token` 与 `LOG_POSITION_ON_TOP=false`。它沿用原生流式日志生命周期：先显示模型与实时耗时，收到首个 chunk 后增加 TTFC，模型步骤结束后增加输入、缓存、输出及思考 Token；多步骤回复会继续增加模型日志行。尾注使用无引用框的 Telegram 等宽文本，实际颜色由客户端主题决定。设置 `SHOW_THINKING_TEXT=false` 可隐藏模型思考/推理摘要，仅保留最终正文。
