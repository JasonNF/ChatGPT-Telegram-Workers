...
Telegram 紧凑尾注可使用 `ENABLE_SHOWINFO=true`、`SHOW_PARTS=model,model_time,first_chunk_time,token` 与 `LOG_POSITION_ON_TOP=false`。它沿用原生流式日志生命周期：先显示模型与实时耗时，收到首个 chunk 后增加 TTFC，模型步骤结束后增加输入、缓存、输出及思考 Token；多步骤回复会继续增加模型日志行。
