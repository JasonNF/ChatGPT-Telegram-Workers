...
For the compact Telegram footer, set `ENABLE_SHOWINFO=true`, `SHOW_PARTS=model,model_time,first_chunk_time,token`, and `LOG_POSITION_ON_TOP=false`. The first line shows the model, total duration, and TTFC; the second shows input, cached, output, and reasoning tokens.
