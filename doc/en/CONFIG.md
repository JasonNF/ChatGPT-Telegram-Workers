...
| TELEGRAM_THINKING_DRAFT_ENABLED | Editable thinking animation     | `true`                                          | Show the animation in an editable message and reuse the same bubble for the final answer                                                                                                                                                                                                                 |
| TELEGRAM_THINKING_INTERVAL   | Thinking animation interval       | `900` (number)                                  | Dot animation frame interval in milliseconds                                                                                                                                                                                                                                                            |
| TELEGRAM_THINKING_LABEL      | Thinking indicator label          | `''`                                            | Optional label before the animated dots; leave empty to show only the icon and dots                                                                                                                                                                                                                      |
| TELEGRAM_THINKING_CUSTOM_EMOJI_ID | MOSS animated icon           | `''`                                            | Telegram `custom_emoji_id` used by both the thinking animation and final reply suffix; an empty value uses the red-circle thinking fallback                                                                                                                                                              |
...
For the compact Telegram footer, set `ENABLE_SHOWINFO=true`, `SHOW_PARTS=model,model_time,first_chunk_time,token`, and `LOG_POSITION_ON_TOP=false`. It follows the native streaming log lifecycle: model and live duration appear first, TTFC is added after the first chunk, and input, cached, output, and reasoning tokens appear when a model step finishes; multi-step replies add model log lines progressively. The footer uses unquoted Telegram monospace text, whose exact color is controlled by the client theme. Set `SHOW_THINKING_TEXT=false` to hide the reasoning summary and keep only the final answer.
...
| SHOW_PARTS                | Display Information Section | `['model', 'model_time', 'token', 'tool', 'tool_time', 'first_chunk_time]`                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Display Information Section                                                                                                                                                                                                                                                                                             |
